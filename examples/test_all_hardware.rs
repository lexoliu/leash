...
async fn main() -> native_sandbox::Result<()> {
    ...
    let gpu_script = scripts_dir.join("test_gpu.py").to_string_lossy().to_string();
    let npu_script = scripts_dir.join("test_npu.py").to_string_lossy().to_string();
    let hw_script = scripts_dir.join("test_hardware.py").to_string_lossy().to_string();
